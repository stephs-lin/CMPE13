 ...
 *                          should be exactly 2 chars long, plus a null char            (DONE)
 * @param message_event //An event corresponding to the parsed message.                 (DONE)
 *                      //If the message could be parsed successfully,
 *                          message_event's type will correspond to the message type and 
 *                          its parameters will match the message's data fields.        (DONE)
 *                      //If the message could not be parsed,                           (DONE)
 *                          message_events type will be BB_EVENT_ERROR
 * 
 * @return STANDARD_ERROR if:
 *              the payload does not match the checksum                                 (DONE)
 *              the checksum string is not two characters long                          (DONE)
 *              the message does not match any message template                         (DONE ASSUMING IF BB_EVENT.TYPE = ERROR)
 *          SUCCESS otherwise
 * 
 ...
 * @return                   The length of the string stored into 'message_string'.             (DONE)
